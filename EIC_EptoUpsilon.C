{
 ...
 //construct dummy reaction particle
  ...
  TEGParticle* rho=decay1->AddProduct(553,"Upsilon");
 ...
 //make a target
  ...
  Double_t cross_sec=0.05*1E-3;//photoproduction cross section microbarns
  //Double_t Lumin=5E33;
  Double_t Lumin=1E34;
  //  Double_t rate=f2->Integral(Eemin,Eemax,thmin,thmax)*1E35*1E-30*2*TMath::Pi();
  Double_t rate=cross_sec*f2->Integral(Eemin,Eemax,thmin,thmax)*Lumin*1E-30*2*TMath::Pi() * 0.0252; //0.06 branch ratio
  ...
  for(Int_t j=0;j<NFiles;j++){
    ...
    outg->SetFileName(Form("EICdata/EptoUpsilon_t4_%d.ld",j));//Output file name
