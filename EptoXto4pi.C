{
  ...
  Double_t time=1*60*60*24*3*30/1000; //seconds
 ...
 for(Int_t i=0;i<rate*time;i++){
   ...
   if(i%10000==0){bm->Stop("Epto3pi");cout<<"event "<<i<<" "<<bm->GetRealTime("Epto3pi")<<" "<<" last 10000 took "<<bm->GetRealTime("Epto3pi")-timer<< endl;timer=bm->GetRealTime("Epto3pi");bm->Start("Epto3pi");}
