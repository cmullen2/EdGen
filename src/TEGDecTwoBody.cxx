...
Bool_t TEGDecTwoBody::GenWCosThDecay( )
{
  ...
  if(fDist){  
    ...
    TF1* PhiDist=0;
    if(PhiDist=fDist->GetPhiDist()){ //look for a phi modulation dependent on particle polarisation
      PhiDist->SetParameter(0,fMother->GetPol()->Mag()); //hack first parameter must be polarisation
      fPhi  = fDist->GetPhi(); //Add polarisation plane to decay phi
      //fPhi  = fMother->GetPol()->Phi()+fDist->GetPhi(); //Add polarisation plane to decay phi
  // TVector3 zV=CMGamma.Vect().Unit();
  // TVector3 yV=CMBeam.Vect().Cross(CMGamma.Vect()).Unit();
  // TVector3 xV=yV.Cross(zV).Unit();

      
      //cout<<fPhi<<" "<<fMother->GetPol()->Phi()<<" "<<fDist->GetPhi()<<endl;
     }
    else fPhi  = fDist->GetPhi();
  ...
  BoostLab( );
  //cout<<"Boosted"<<fP4d[0]->Phi()<<" "<<fP4d[1]->Phi()<<endl;
  //  cout<<"TEGDecay::Gen2Decay( ) "<<fP4d[0]->E()<<" "<<fP4d[1]->E()<<" "<<fP4d[0]->M()<<" "<<fP4d[1]->M()<<" "<<acos(fCosTheta)*TMath::RadToDeg()<<endl;
  //cout<<"TEGDecay::Gen2Decay( ) "<<fP4d[0]->Theta()*TMath::RadToDeg()<<" "<<fP4d[1]->Theta()*TMath::RadToDeg()<<endl;
