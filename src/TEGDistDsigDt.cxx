...
TEGDistDsigDt::TEGDistDsigDt(Double_t b,Double_t min,Double_t max):TEGDistDsigDOmega(){
  ...
  ftDist=new TF1("DistDsigDt","1000*TMath::Exp(-[0]*x)",0,500);
...
Double_t TEGDistDsigDt::GetCosTh(){
  ...
  //cout<<"react "<<fRP4->M()<<" "<<fRP4->E()<<" "<<fRP4->Rho()<<" "<<fRP4->Theta()<<endl;
  // cout<<"TEGDistDsigDt::GetCosTh() tdisted "<<tmin<<" "<<tmax<<" "<< fRP4->M()<<endl;
 ...
 //Get t from defined distribution
  ...
  // fCosTh=cos(costh1);
  // cout<<"TEGDistDsigDt::GetCosTh() tdisted "<<ft<<" "<< fCosTh<<endl;
  // TVector3 alpha(0,0,1);
  // alpha.SetTheta(TMath::ACos(fCosTh));
  // TVector3 vY=fGP4->Vect().Cross(TVector3(0,0,1)).Unit();
  // alpha.Rotate(fGP4->Theta(),-vY);
  // fCosTh=alpha.CosTheta();
