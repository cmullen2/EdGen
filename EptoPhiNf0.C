{
  ...
  outg->SetFileName("../EdGenOut/EptoPhiNf03months.root");
  ...
  TEGParticle* phi=decay1->AddProduct(333,"X");//an EGMeson
  //X->SetIsTrack(kFALSE);//don't track this
  //TF1* Xmass=new TF1("Xmass","pol0(0)",0.5,4.0);
  //Xmass->SetParameter(0,1);
  //X->SetMassDist(Xmass);
  TEGParticle* pr=decay1->AddProduct(2212,"proton");
  pr->FixCurrDecay(0);
  ...
  TEGParticle* gamma1=decayPi0->AddProduct(22,"gamma1pi0");
  TEGParticle* gamma2=decayPi0->AddProduct(22,"gamma2pi0");
  TEGDecay* decayEta=new TEGDecay();
  eta->AddDecayChannel(decayEta,1);
  TEGParticle* gamma3=decayEta->AddProduct(22,"gamma1eta");
  TEGParticle* gamma4=decayEta->AddProduct(22,"gamma2eta");
  ...
  Double_t cross_sec=3;//photoproduction cross section microbarns
  Double_t rate=cross_sec*f2->Integral(Eemin,Eemax,thmin,thmax)*1E35*1E-30*2*TMath::Pi();
  Double_t time=1*60*60*24*3*30/10*0.3931; //seconds
