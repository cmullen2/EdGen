{
  ...
  outg->SetFileName("EptoEtapp.root");
  ...
  TEGParticle* etap=decay1->AddProduct(211,"etap");//a pi+
  etap->SetIsTrack(kFALSE);//don't track this
  TEGParticle* proton=decay1->AddProduct(2212,"proton");
  pr->FixCurrDecay(0);

 TEGDecay* decayEtap=new TEGDecay();
 etap->AddDecayChannel(decayEtap,1);
  TEGParticle* gamma=decayEtap->AddProduct(22,"gamma");
  TEGParticle* eplus=decayEtap->AddProduct(-11,"elp");
 TEGParticle* eminus=decayEtap->AddProduct(11,"elm");
  ...
  Double_t Eemax=9;//max scattered electron energy
  vPhot->SetERange(Eemin,Eemax);  //set eprime E range
  Double_t thmin=8*TMath::DegToRad(); //min scatter angle convert to rad
  Double_t thmax=30*TMath::DegToRad(); //max scatter angle convert to rad
  ...
  Double_t cross_sec=100;//total photoproduction cross section microbarns
  // Double_t cross_sec=0.05*4.69E-4;
  //Double_t Lumin=5E33;
  Double_t Lumin=1E35;
