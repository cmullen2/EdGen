{
 ...
 //construct dummy reaction particle
  ...
  TEGParticle* em=decay1->AddProduct(11,"em");
  TEGParticle* ep=decay1->AddProduct(-11,"ep");
 ...
 //make a target
  ...
  Double_t thmin=1./100000; //min scatter angle convert to rad
  //  Double_t thmax=1./100; //max scatter angle convert to rad
  Double_t thmax=0.05*TMath::DegToRad(); //max scatter angle convert to rad
  ...
  Double_t cross_sec=1000*1E-3;//photoproduction cross section microbarns
  //Double_t Lumin=5E33;
  Double_t Lumin=1E34;
  //  Double_t rate=f2->Integral(Eemin,Eemax,thmin,thmax)*1E35*1E-30*2*TMath::Pi();
  Double_t rate=cross_sec*f2->Integral(Eemin,Eemax,thmin,thmax)*Lumin*1E-30*2*TMath::Pi() * 0.02; //0.06 branch ratio
  ...
  for(Int_t j=0;j<NFiles;j++){
    ...
    outg->SetFileName(Form("EICdata/EptoepemPS_%d.ld",j));//Output file name
