{
  ...
  outg->SetFileName("../EdGenOut/EptoOmegaKKK_1M.root");
  ...
  TEGParticle* Omega=decay1->AddProduct(3334,"Omega");
  TEGParticle* Kp1=decay1->AddProduct(321,"Kp1");
  TEGParticle* Kp2=decay1->AddProduct(321,"Kp2");
  TEGParticle* K0=decay1->AddProduct(310,"K0");//note this is K0S pdg
  TEGDecay* decayK0=new TEGDecay();
  K0->AddDecayChannel(decayK0,1);
  TEGParticle* pip=decayK0->AddProduct(211,"pip");
  TEGParticle* pim=decayK0->AddProduct(-211,"pim"); 
  K0->SetIsTrack(kFALSE);

  //decay Omega
  TEGDecay* decayOm=new TEGDecay();
  Omega->AddDecayChannel(decayOm,1);
  TEGParticle* Lambda=decayOm->AddProduct(3122,"Lambda");
  TEGParticle* Km=decayOm->AddProduct(-321,"Km"); 
  Omega->SetIsTrack(kFALSE);
  //decay Lambda
 ...
 //make the beam
  ...
  Double_t Eemax=6.0;//max scattered electron energy
  ...
  Double_t time=1000*60.*60.; //seconds
